...
import { OWN_SHOP_CORNER_REVENUE_GROWTH_PER_CORNER } from "../libs/query";
import CardComponent from "../libs/card";

interface OwnShopCornerOrderGrowthPerCornerProps {
  filterParams: {
    periodic?: string;
    startDate?: string;
    endDate?: string;
...
const OwnShopCornerOrderGrowthPerCorner: React.FC<
  OwnShopCornerOrderGrowthPerCornerProps
> = ({ filterParams }) => {
  ...
  const { loading, error, data } = useQuery(
    OWN_SHOP_CORNER_REVENUE_GROWTH_PER_CORNER,
    {
      variables: {
        startDate: startDate,
        endDate: endDate,
  ...
  if (error || !data?.getRevenueGrowthPerCorner?.length)
    return <p>No data available</p>;

  const getLabels = Array.from(
    new Set(
      data.getRevenueGrowthPerCorner.map(
  ...
  data.getRevenueGrowthPerCorner.forEach((datas: { sourcetype: any }) => {
  ...
  sourceTypes.forEach((sourcetype) => {
    const filledMonths = data.getRevenueGrowthPerCorner
      .filter((datas: { sourcetype: any }) => datas.sourcetype === sourcetype)
      .map((datas: { periode: any }) => datas?.periode);

    const getDataOrder = data.getRevenueGrowthPerCorner
      .filter((datas: { sourcetype: any }) => datas.sourcetype === sourcetype)
  ...
  return (
    <CardComponent
      ...
      title={"Own Shop Order Growth Per Corner"}
