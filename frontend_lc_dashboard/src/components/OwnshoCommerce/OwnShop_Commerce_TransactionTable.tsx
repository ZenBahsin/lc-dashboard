...
import { OWN_SHOP_COMMERCE_TABLE_MATRIX } from "../libs/query";
...
const OwnShopCommerceTransactionTable: React.FC<Props> = ({ filterParams }) => {
  ...
  const {
    ...
    error,
    data: queryData,
  } = useQuery(OWN_SHOP_COMMERCE_TABLE_MATRIX, {
    variables: {
      startDate,
      endDate,
    },
  ...
  useEffect(() => {
    if (queryData && queryData.getMatrixTableofOwnShopCommerceTransaction) {
      setData(
        processData(queryData.getMatrixTableofOwnShopCommerceTransaction)
      );
      setDataTotal(
        processData(queryData.getMatrixTableofTotalOwnShopCommerceTransaction)
      );
    }
  }, [queryData]);
  ...
  return (
    <CardComponent
      title={"Table Transaction Commerce"}
