...
import { B2B_PRODUCT_CONTRIBUTION } from "../libs/query";
...
const B2BProductContribution: React.FC<ProductContributionProps> = ({
  filterParams,
}) => {
  ...
  const { loading, error, data } = useQuery(B2B_PRODUCT_CONTRIBUTION, {
    variables: {
      startDate: startDate,
      endDate: endDate,
    },
  ...
  const onConfirmHandler = (notes: any) => {
    exportPDF({
      elementId: "b2bproductcontribution",
      startDate,
      endDate,
      periodic,
      printFullPage: true,
  ...
  return (
    <CardComponent
      title={"B2B Product Contribution"}
      onConfirmHandler={onConfirmHandler}
      data-html2canvas-ignore="true"
    >
      <Box
        id="b2bproductcontribution"
        style={{
          height: "60vh",
          position: "relative",
          marginBottom: "1%",
