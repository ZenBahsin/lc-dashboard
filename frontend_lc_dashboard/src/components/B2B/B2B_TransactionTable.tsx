...
import { B2B_TRANSACTION_TABLE_MATRIX } from "../libs/query";
...
const B2BTransactionTable: React.FC<Props> = ({ filterParams }) => {
  ...
  const {
    ...
    error,
    data: queryData,
  } = useQuery(B2B_TRANSACTION_TABLE_MATRIX, {
    variables: {
      startDate,
      endDate,
    },
  ...
  useEffect(() => {
    if (queryData) {
      setData(processData(queryData.getMatrixTableofB2BTransaction));
      setDataTotal(processData(queryData.getMatrixTableofB2BTotalTransaction));
    }
  }, [queryData]);
...
export default B2BTransactionTable;
