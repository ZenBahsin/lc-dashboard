...
import { B2B_RETAIL_TRANSACTION_TABLE_MATRIX } from "../libs/query";
...
const B2BRetailTransactionTable: React.FC<Props> = ({ filterParams }) => {
  ...
  const {
    ...
    error,
    data: queryData,
  } = useQuery(B2B_RETAIL_TRANSACTION_TABLE_MATRIX, {
    variables: {
      startDate,
      endDate,
    },
  ...
  useEffect(() => {
    if (queryData) {
      setData(processData(queryData.getMatrixTableofB2BRetailTransaction));
      setDataTotal(
        processData(queryData.getMatrixTableofTotalB2BRetailTransaction)
      );
    }
  }, [queryData]);
  ...
  return (
    <CardComponent
      title={"B2B Retail Table Transaction"}
...
export default B2BRetailTransactionTable;
