...
import { B2B_CORPORAE_TRANSACTION_TABLE_MATRIX } from "../libs/query";
...
const B2BCorpTransactionTable: React.FC<Props> = ({ filterParams }) => {
  ...
  const {
    ...
    error,
    data: queryData,
  } = useQuery(B2B_CORPORAE_TRANSACTION_TABLE_MATRIX, {
    variables: {
      startDate,
      endDate,
    },
  ...
  useEffect(() => {
    if (queryData) {
      setData(
        processData(queryData.getMatrixTableofB2BCorporateTransaction)
      );
      setDataTotal(
        processData(queryData.getMatrixTableofTotalB2BCorporateTransaction)
      );
    }
  }, [queryData]);
  ...
  return (
    <CardComponent
      title={"B2B Corporate Table Transaction"}
