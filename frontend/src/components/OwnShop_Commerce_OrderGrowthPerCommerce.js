...
const OwnShopCommerceOrderGrowthPerCommerce = () => {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(
      `http://localhost:8000/api/revenuegrowthpercommerce?startdate=${startdate}&enddate=${enddate}&groupby=${groupby}`
    )
      .then((res) => res.json())
      .then((data) => {
        setData(data);
  ...
  if (isLoading) return <p>Loading Oerder Growth per Commerce...</p>;
  if (!data) return <p>No data</p>;

  const getLabels = Array.from(
    new Set(
      data.getDataRevenueGrowthPerCommerce.map((datas) =>
        groupby === "Week"
          ? datas?.Minggu
          : groupby === "Month"
          ? datas?.bulan
  ...
  const sourceTypes = [];
  data.getDataRevenueGrowthPerCommerce.forEach((datas) => {
  ...
  sourceTypes.forEach((sourcetype) => {
    const filledMonths = data.getDataRevenueGrowthPerCommerce
    ...
    const getDataOrder = data.getDataRevenueGrowthPerCommerce
      .filter((datas) => datas.sourcetype === sourcetype)
