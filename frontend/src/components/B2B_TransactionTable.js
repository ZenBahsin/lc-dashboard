...
const B2BTransactionTable = () => {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(
      `http://localhost:8000/api/tablematrixofb2btransaction?startdate=${startdate}&enddate=${enddate}`
    )
      .then((res) => res.json())
      .then((data) => {
        setData(data);
  ...
  data.getMatrixTableofB2BTransactionData.forEach((data) => {
  ...
  data.getMatrixTableofB2BTotalTransactionData.forEach((data) => {
